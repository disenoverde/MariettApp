...
export default function HistoriaSalud({ clienteId }) {
  ...
  const loadHistoria = async () => {
    try {
      // Cargado con onSnapshot en useEffect

      if (data) {
        setHistoria(data)
        setFormData(data)
      } else {
        setEditing(true) // Si no hay historia, activar modo edición
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (historia) {
        // Actualizar
        await setDoc(doc(db, 'historias_salud', clienteId), { ...dataToSave, updated_at: serverTimestamp() }, { merge: true })

        if (error) throw error
      } else {
        // Insertar
        await setDoc(doc(db, 'historias_salud', clienteId), { ...dataToSave, created_at: serverTimestamp(), updated_at: serverTimestamp() })
  ...
  return (
    <div className="card">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {/* Salud Personal e historia familiar */}
        ...
        {/* Información sobre Salud Física */}
